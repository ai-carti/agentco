...
class TaskRepository(BaseRepository[TaskORM, Task]):
    ...
    def list_by_company(self, company_id: str, limit: int | None = None, offset: int | None = None) -> list[Task]:
        return self.list(limit=limit, offset=offset, order_by=TaskORM.created_at.asc(), company_id=company_id)

    def list_by_agent(self, agent_id: str, limit: int | None = None, offset: int | None = None) -> list[Task]:
        return self.list(limit=limit, offset=offset, order_by=TaskORM.created_at.asc(), agent_id=agent_id)
