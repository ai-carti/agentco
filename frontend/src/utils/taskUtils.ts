...
// SIRI-UX-423: clamp negative diff to 0 — future timestamps (clock skew) yielded "-5s ago"
export function relativeTime(iso: string): string {
  const rawDiffMs = Date.now() - new Date(iso).getTime()
  if (isNaN(rawDiffMs)) return '?'
