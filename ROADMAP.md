...
| SIRI-UX-133 | minor | **`WarRoomPage.tsx` feed messages: `role="button"` без `aria-label`** — элементы сообщений с `role="button"` не имеют явного `aria-label`. Screen reader озвучивает весь текст сообщения как имя кнопки. При длинных сообщениях (>120 символов) это плохой UX. Фикс: добавить `aria-label={`Expand message from ${msg.senderName}`}` на expandable элементы. `WarRoomPage.tsx:~640` | Siri | open |
| SIRI-UX-134 | minor | **`WarRoomPage.tsx` `aria-expanded` не устанавливается в `false` для не-длинных сообщений** — элементы с `isLong=false` не получают `aria-expanded` вообще (`aria-expanded={undefined}`). По WAI-ARIA, отсутствие атрибута и `aria-expanded="false"` — разные состояния. Если future рефактор добавит role="button" на все сообщения, это создаст AT-баг. Текущее поведение: корректное для AT (нет aria-expanded → не expandable). Но тест в `SIRI-UX-128-130.test.tsx` использует `aria-expanded="false"` как ожидание — это работает только когда msg expandable (`isLong=true`). Риск: тесты проверяют поведение при `isLong=true` — OK. Зафиксировать в документации. | Siri | open |
| SIRI-UX-135 | major | **`FeedMessage` интерфейс не имеет полей `senderId`/`targetId`** — `warRoomStore.ts:FeedMessage` объявляет только `senderName`, `targetName`, без `senderId`/`targetId`. Однако `useWarRoomSocket.ts:79` передаёт `data as unknown as FeedMessage` — `as unknown` подавляет ошибку типа. Если бэкенд пришлёт `sender_id`/`target_id`, они будут проигнорированы. Добавить `senderId?: string; targetId?: string` к интерфейсу + убрать `as unknown` cast в хуке. `warRoomStore.ts:14-21`, `useWarRoomSocket.ts:79` | Siri | open |
