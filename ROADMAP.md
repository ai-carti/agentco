...
| ALEX-TD-083 | minor | **`memory/service.py:92` — `MemoryService.get_all()` — синхронный blocking SQLite вызов в async контексте без `run_in_executor`**. `save_memory()` и `get_relevant_memories()` корректно оборачивают sqlite в executor (ALEX-TD-021), но `get_all()` вызывает `self._store.get_all()` напрямую — блокирует event loop. В FastAPI endpoint `handlers/memory.py:68` это вызывается в синхронном `def` (не async), что допустимо для HTTP handler'а, но если `get_all` будет вызван из async context (будущий рефактор), это станет блокирующим вызовом. Fix: либо добавить `run_in_executor` в `MemoryService.get_all()`, либо пометить явным комментарием "sync-only". `memory/service.py:92` | Alex | open |
| ALEX-TD-084 | minor | **`handlers/ws_events.py:_watch_disconnect()` — `websocket.receive()` возвращает данные если клиент присылает message, а не закрывает соединение**. `_watch_disconnect` вызывает `receive()` в loop, но `receive()` не бросает исключение при обычных data messages — только при `WebSocketDisconnect`. Если клиент посылает данные (ping frame, heartbeat), `_watch_disconnect` завершится нормально (не бросит) и `asyncio.wait(FIRST_COMPLETED)` прекратит работу `forward_task` → WS соединение закрывается при первом клиентском сообщении. `handlers/ws_events.py:_watch_disconnect:101`. | Alex | open |
| ALEX-TD-085 | minor | **`orchestration/agent_node.py:370` — `except Exception as e` глотает error без propagation**. При ошибке LLM-вызова возвращается `{"status": "error", "error": str(e)}` но не re-raise. LangGraph граф продолжит работу с `status="error"` в state — CEO node не проверяет этот статус при синтезе результатов. Риск: граф завершится как `completed` несмотря на ошибку LLM. Fix: проверить `status=error` в `should_continue` edge условии или re-raise исключение. `orchestration/agent_node.py:369-374` | Alex | open |
