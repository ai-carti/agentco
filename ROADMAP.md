...
| BUG-056 | minor | `SettingsPage.tsx`: GET /credentials возвращает 403 → silent fail. `res.ok ? res.json() : []` при 403 просто ставит `credentials=[]` без показа ошибки пользователю. Юзер видит пустой список не понимая почему. Фикс: добавить обработку !res.ok с setCredentialsError + показом в UI. `SettingsPage.tsx:~113` | Siri | open |
