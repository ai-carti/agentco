...
| ALEX-TD-129 | minor | **`handlers/memory.py:get_agent_memory` — синхронный `memory_service.get_all()` вызывается из синхронного FastAPI handler**: handler объявлен как `def` (не `async def`), что заставляет FastAPI запускать его в thread pool. Однако остальной код memory/service.py использует `async def` с `run_in_executor` для блокирующих sqlite вызовов. Несогласованность: если handler когда-либо переделают в `async def`, `get_all()` заблокирует event loop. Фикс: добавить `async def` обёртку через `run_in_executor` для `get_all` в `MemoryService` и сделать `get_agent_memory` sync+threadpool-safe (или явно задокументировать почему sync-handler здесь корректен). `handlers/memory.py:31`, `memory/service.py:92` | Alex | open |
| ALEX-TD-130 | major | **`orchestration/agent_node.py:agent_node` — при LLM ошибке возвращает `{"status": "error", "error": str(e)}` без обновления `run.status` в БД**: верхний `except Exception` в `agent_node` возвращает dict с `status=error`, но `execute_run` ожидает что граф обновит `state["status"]`. Если граф завершается с ошибкой через этот путь, `execute_run` может не поймать final_status и Run навсегда зависнет в `running`. Примечание: `execute_run` имеет outer except который поймает re-raise из LangGraph — но если граф вернёт status=error без исключения, final_status="error" будет корректно обработан через `if final_status in ("failed", "error")`. Реальный риск: LangGraph может не перебросить ошибку из ноды в outer except. `orchestration/agent_node.py:380-384`, `services/run.py:execute_run` | Alex | open |
