...
| ALEX-TD-054 | minor | `handlers/companies.py:update_company` — двойной DB-запрос: сначала `CompanyService(session).get()`, затем снова `CompanyService(session).update()` — оба делают SELECT на одну запись. N-вызов паттерн при каждом PUT /companies/{id}. Фикс: объединить в один вызов `update()` который сразу делает ownership check + update. | Alex | open |
| ALEX-TD-055 | minor | `handlers/ws_events.py` — при неправильном/просроченном токене WebSocket закрывается до `accept()`. Некоторые клиенты/proxies (Nginx, HAProxy) логируют это как ошибку соединения. Правильный паттерн: `await websocket.accept()` → затем `close(code=1008)`. Также: код 1008 не всегда корректно обрабатывается в браузерах — лучше 4001/4003. Низкий приоритет, но clean pattern. | Alex | open |
| ALEX-TD-056 | minor | `services/run.py:_execute_agent` — import inside function: `import asyncio as _asyncio` и `import os as _os` делаются при каждом вызове (внутри цикла по попыткам). Python кэширует импорты, но это anti-pattern — читаемость страдает. Фикс: поднять импорты на уровень модуля. | Alex | open |
| ALEX-TD-057 | minor | `handlers/companies.py:get_company/delete_company/update_company` — дублированный паттерн: `get()` + проверка `owner_id` в 3 местах. При добавлении нового endpoint легко забыть проверку. Фикс: перенести ownership check в `CompanyService` (уже делается в `RunService` через `create_with_goal`/`list_by_company_owned`). Рефактор без изменения поведения. | Alex | open |
