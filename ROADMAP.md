...
| PRE-DEMO-002 | critical | Final backend deployment check перед демо: Alex проверяет: 1) Railway deployment — зайти через `railway status` или gh run list, убедиться что последний deploy green; 2) Задокументировать чётко в `/home/clawdbot/projects/agentco/qa-report/PRE-DEMO-002-checklist.md` два критических human-action пункта: (a) GitHub Secret `VITE_API_URL` должен быть задан = Railway backend URL; (b) Railway Variable `AGENTCO_DB_URL` должен указывать на persistent volume `sqlite:////data/agentco.db`. 3) Попробовать задать эти переменные через CLI если gh/railway CLI доступны; 4) Прогнать 403 тесты (`uv run pytest -q`), убедиться green; 5) Отправить Telegram (target=667566350): список что проверено + что требует action от @timofeytst. | Alex | open |
