...
| SIRI-UX-103 | minor | `AuthPage.tsx` — Sign In / Sign Up tabs реализованы как `<button>` без `role="tab"`, `role="tablist"`, и `aria-selected`. Screen reader не анонсирует переключение вкладок. Стандарт ARIA Authoring Practices Guide (APG) требует tablist pattern для tab-switching UI. | Siri | open |
