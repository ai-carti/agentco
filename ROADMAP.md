...
|----|----------|----------|----------|--------|
| ALEX-TD-205 | minor | **`orchestration/agent_node.py:_build_messages_with_memory` — memory inject ignores `total_tokens` limit**: перед вызовом LLM функция добавляет воспоминания агента в system_prompt без учёта `MAX_RUN_TOKENS` лимита. Если агент накопил много воспоминаний, суммарные токены (воспоминания + сообщения) могут превысить лимит ещё до вызова LLM. Граф обнаружит превышение только на следующей итерации после LLM ответа — это один лишний (дорогой) вызов. Фикс: в `_build_messages_with_memory` проверять `state.get("total_tokens", 0) >= _get_max_tokens()` и пропускать inject если лимит уже достигнут. `orchestration/agent_node.py:_build_messages_with_memory` | Alex | open |
| ALEX-TD-206 | minor | **`services/run.py:_execute_agent` — retry count не сбрасывается между вызовами `_execute_agent`**: если `execute_run` вызывается повторно (например, через `create_and_start` с restart), счётчик попыток в `_execute_agent` начинается с нуля — это корректно. Но если `_NO_RETRY_ERRORS` не совпадает с ошибкой, все три попытки расходуются на одну ноду. При multi-agent графах (CEO → 3 subagents) общий retry budget: 3 попытки × N агентов. Не критично, но retry-стратегия не документирована в .env.example. Фикс: добавить комментарий в .env.example объясняющий что `RUN_MAX_RETRIES` применяется per-execute_run call, не per-agent-step. `backend/.env.example` | Alex | open |
| ALEX-TD-207 | minor | **`handlers/runs.py:list_runs` — `company_id` URL param не проверяется на формат UUID**: передача `company_id="'; DROP TABLE companies;--"` в URL path не вызывает SQL injection (ORM использует parameterized queries), но создаёт бессмысленный DB lookup с гарантированным NOT FOUND. `company_id` в идеале должен проходить UUID-валидацию до DB-запроса. Альтернатива: изменить тип path param на `uuid.UUID` в FastAPI — автоматически вернёт 422 для невалидного UUID. Затрагивает все endpoints с `company_id`/`agent_id`/`task_id` path params. Низкий риск (ORM защищает от injection), но улучшает UX (422 вместо 404 при случайных опечатках). `handlers/runs.py, handlers/agents.py, handlers/tasks.py` | Alex | open |
| SIRI-UX-311 | minor | **`TaskDetailSidebar.tsx:sidebar-backdrop` — backdrop div без `role`/`tabIndex`/`onKeyDown`**: backdrop-div закрывается по `onClick`, но не имеет `role="button"`, `tabIndex={0}`, `aria-label`, `onKeyDown` для Enter/Space/Escape. Клавиатурный пользователь не может закрыть sidebar через backdrop. SIRI-UX-278 уже зафиксировал этот паттерн для `Sidebar.tsx:sidebar-backdrop` — `TaskDetailSidebar` пропущен. Fix: добавить `role="button"`, `tabIndex={0}`, `aria-label="Close task details"`, `onKeyDown` handler. `TaskDetailSidebar.tsx:sidebar-backdrop` | Siri | fixed |
| SIRI-UX-312 | minor | **`CompanyPage.tsx:handleLoadMoreTasks:250` — `setTaskOffset(taskOffset + items.length)` stale closure**: `setTaskOffset(taskOffset + items.length)` захватывает `taskOffset` из closure — при concurrent вызовах или StrictMode double-invoke значение может быть устаревшим. Паттерн с functional updater `setTaskOffset(prev => prev + items.length)` гарантирует атомарность. Аналогичная проблема (stale closure в setState) уже зафиксирована в SIRI-UX-219 (setAgents), SIRI-UX-225 (setTasks). Fix: заменить на `setTaskOffset(prev => prev + items.length)`. `CompanyPage.tsx:handleLoadMoreTasks:250` | Siri | fixed |
| SIRI-UX-313 | minor | **`warRoomStore.ts:loadMockData:172` — `cost: 0.042` нарушает инвариант "cost из WS"**: `loadMockData()` устанавливает `cost: 0.042` — ненулевой mock cost. Это противоречит SIRI-POST-004 + SIRI-UX-125 + SIRI-UX-212 которые установили что cost должен приходить исключительно из WS `llm_token` событий. В dev режиме (VITE_MOCK_WAR_ROOM=true) cost counter показывает $0.0420 без реальных LLM вызовов, маскируя баги в cost accumulation логике. Fix: заменить `cost: 0.042` на `cost: 0` в `loadMockData`. `warRoomStore.ts:loadMockData` | Siri | fixed |
| SIRI-UX-314 | minor | **`GlobalSearch.tsx` — поиск агентов/задач ограничен текущей компанией без объяснения пользователю**: `results` строятся из `agentStore.agents` и `agentStore.tasks` — которые содержат данные только текущей компании. Если пользователь открывает поиск с главной страницы (`/`) или после logout/reload, `currentCompany` = null и поиск по агентам/задачам возвращает 0 результатов даже при наличии данных. Нет ни subtitle "Searching in current company only", ни hint что нужно выбрать компанию. Fix: добавить subtitle под input когда `currentCompany` не null — "Searching in {company.name}"; когда null — показывать hint "Select a company to search agents and tasks". `GlobalSearch.tsx:search scope` | Siri | fixed |
| SIRI-UX-310 | minor | **`AuthPage.tsx` — tablist без Arrow key navigation**: `role="tablist"` имеет roving tabindex (SIRI-UX-282), но нет ArrowLeft/ArrowRight обработчиков для переключения между вкладками. WAI-ARIA APG Tabs Pattern требует стрелочную навигацию. CompanyPage уже реализует это (SIRI-UX-143). Fix: добавить `onKeyDown` на оба tab-button: ArrowRight на Sign In переводит фокус на Sign Up, ArrowLeft на Sign Up — обратно. `AuthPage.tsx:tablist` | Siri | fixed |
| SIRI-UX-309 | minor | **`CompanySettingsPage.tsx:handleDelete` — нет AbortController**: async DELETE fetch без AbortController. При навигации прочь во время удаления компании `toast.success/error` и `setIsDeleting(false)` вызываются на unmounted component — React warning. Fix: добавить `deleteAbortRef`, создать AbortController в `handleDelete`, передать `signal` в fetch, защитить все setState/toast с `signal.aborted` guard. Cleanup добавлен в useEffect. `CompanySettingsPage.tsx:handleDelete` | Siri | fixed |
| SIRI-UX-308 | minor | **`SIRI-UX-304-306-307.test.tsx:140-143` — Node.js `fs`/`path`/`__dirname` без `@types/node`**: SIRI-UX-307 тест использовал `import('fs')`, `import('path')`, `__dirname` для чтения исходника KanbanBoard. Вызывал 3 TS ошибки в `tsc --noEmit`. Тот же паттерн что уже фиксировался в SIRI-UX-269, SIRI-UX-286. Fix: переписан как поведенческий React rendering тест (рендер KanbanBoard, проверка что TaskCard отображает задачу из store). `src/__tests__/SIRI-UX-304-306-307.test.tsx:SIRI-UX-307 test` | Siri | fixed |
| SIRI-UX-289 | minor | **`WarRoomPage.tsx` — `ws-error-banner` и `run-status-banner` без `role="alert"`**: оба банера появляются динамически (WS ошибка + окончание рана), но у них нет `role="alert"`. Screen reader не анонсирует их автоматически — пользователь с нарушениями зрения не узнает об ошибке или завершении рана. Аналогичная проблема уже фиксилась в AuthPage (SIRI-UX-283), CompaniesPage (SIRI-UX-148). Fix: добавить `role="alert"` на оба div. `WarRoomPage.tsx:ws-error-banner, run-status-banner` | Siri | fixed |
| SIRI-UX-290 | minor | **`CompanyPage.tsx` — `TASK_LIMIT = 50` объявлен внутри компонента**: константа пересоздаётся при каждом рендере. Аналогичная проблема была в WarRoom.tsx (SIRI-UX-253, исправлено `MAX_RUNS`). Fix: вынести `TASK_LIMIT` на уровень модуля. `CompanyPage.tsx:118` | Siri | fixed |
| SIRI-UX-291 | minor | **`CompanyPage.tsx` — `handleCreateAgent` не мемоизирован**: async функция пересоздаётся при каждом рендере и передаётся как prop в `AgentForm` → лишние re-render AgentForm. `handleLoadMoreTasks` мемоизирован через useCallback (SIRI-UX-277), но `handleCreateAgent` нет. Fix: обернуть в `useCallback` с deps `[id, toast]`. `CompanyPage.tsx:264` | Siri | fixed |
| SIRI-UX-292 | minor | **`WarRoom.tsx` — reconnect без exponential backoff**: при обрыве WS переподключение через фиксированный `setTimeout(connect, 3000)` — нет экспоненциального бэкофа. При длительном отсутствии сервера накапливаются частые неудачные попытки. `useWarRoomSocket.ts` уже реализует backoff (SIRI-UX-116), `WarRoom.tsx` нет. Fix: добавить `retryDelayRef` с экспоненциальным ростом до `MAX_BACKOFF_MS` — аналогично `useWarRoomSocket`. `WarRoom.tsx:connect/onclose` | Siri | fixed |

| SIRI-UX-293 | minor | **`KanbanBoard.tsx:441` — `runError` без `role="alert"`**: ошибка запуска таска появляется динамически внутри `<p>` без `role="alert"` — screen reader не анонсирует её. Fix: добавить `role="alert"` на `<p data-testid="run-error-{id}">`. `KanbanBoard.tsx:441` | Siri | done |
| SIRI-UX-294 | minor | **`WarRoomPage.tsx:436-437` — Дублированное условие отключения Stop-кнопки**: `runStatus === 'idle' \|\| runStatus === 'done' \|\| runStatus === 'failed' \|\| runStatus === 'stopped'` повторяется дважды — в `disabled` и в `style.opacity`. DRY-нарушение: при добавлении нового статуса нужно обновлять в двух местах. Fix: вынести в `const isStopDisabled = stopping \|\| runStatus === ...` перед JSX. `WarRoomPage.tsx:436-437` | Siri | done |

| SIRI-UX-295 | minor | **`GlobalSearch.tsx:9` — `navigator.platform` deprecated**: используется для определения macOS. MDN пометил `navigator.platform` как deprecated с 2022. Правильная альтернатива: `navigator.userAgentData?.platform` (Chrome 90+) с fallback на `navigator.platform`. Fix: обновить `getIsMac()` — попробовать `userAgentData.platform` первым, fallback `navigator.platform`. `GlobalSearch.tsx:9-10` | Siri | fixed |
| SIRI-UX-296 | minor | **`GlobalSearch.tsx:151` — `flatResults` — избыточный псевдоним**: `const flatResults = results` — буквально тот же reference что и `results`. Появился при рефакторинге (вероятно планировалась flatten-логика, которую не добавили). Используется в `handleKeyDown` и рендере — просто `results` чище. Fix: заменить все `flatResults` на `results`, удалить алиас. `GlobalSearch.tsx:151` | Siri | done |
| SIRI-UX-297 | minor | **`WarRoomPage.tsx:31-41` — 10 отдельных подписок на `useWarRoomStore`**: каждый вызов `useWarRoomStore((s) => s.xxx)` создаёт отдельную Zustand-подписку → компонент может ре-рендериться до 10 раз при батчевых обновлениях стора. Zustand поддерживает объединение через `useShallow` / один селектор. Fix: объединить `agents`, `messages`, `cost`, `runStatus`, `flashingAgents` в один `useWarRoomStore(useShallow(s => ({...})))`. Экшены (`addMessage`, `updateAgentStatus` и т.д.) не меняются — их можно оставить отдельно (они стабильны). `WarRoomPage.tsx:31-41` | Siri | fixed |
| SIRI-UX-298 | minor | **`useWarRoomSocket.ts` — `events` state never consumed, triggers re-renders on every WS message**: хук возвращает `{ events, isConnected, error }`, но `WarRoomPage` деструктурирует только `{ isConnected, error }`. Тем не менее `setEvents` вызывается на каждое WS-сообщение → `useWarRoomSocket` ре-рендерится → `WarRoomPage` тоже ре-рендерится. Это бесплатные лишние рендеры пропорциональные трафику в `onmessage`. Fix: убрать `events` state из хука (или переместить в ref если понадобится позже для отладки). `useWarRoomSocket.ts:events` | Siri | fixed |
| SIRI-UX-299 | minor | **`KanbanBoard.tsx` — `handleDragLeave` не проверяет `relatedTarget` — border мигает при перетаскивании через child-элементы**: когда карточка перетаскивается в колонку, внутри колонки находятся дочерние div-ы (TaskCard). Браузер стреляет `dragleave` на колонке при переходе на дочерний элемент → `setDragOverCol(null)` → синяя рамка мигает. Fix: в `handleDragLeave` проверить `e.relatedTarget` — если цель находится внутри колонки, не сбрасывать `dragOverCol`. `KanbanBoard.tsx:handleDragLeave` | Siri | fixed |
| SIRI-UX-300 | minor | **`AgentEditPage.tsx` — `saveError` без `role="alert"`**: при ошибке сохранения агента рендерится `<p data-testid="agent-edit-error">` без `role="alert"`. Screen reader не анонсирует ошибку автоматически — пользователь не узнает о проблеме. Аналогичный паттерн исправлен в AuthPage (SIRI-UX-283), CompaniesPage (SIRI-UX-148). Fix: добавить `role="alert"` на `<p data-testid="agent-edit-error">`. `AgentEditPage.tsx:173` | Siri | fixed |
| SIRI-UX-301 | minor | **`AgentPage.tsx` — `historyError` без `role="alert"`**: динамическая ошибка загрузки истории задач (`<p data-testid="history-load-error">`) без `role="alert"` — screen reader не анонсирует. Fix: добавить `role="alert"`. `AgentPage.tsx:341` | Siri | fixed |
| SIRI-UX-302 | minor | **`TaskDetailSidebar.tsx` — локальная `formatDate()` дублирует логику из `taskUtils`**: `formatDate(iso)` определена локально в сайдбаре с форматом `'Mar 25, 2026'`, тогда как `taskUtils.formatDueDate()` возвращает `{ label, overdue }` с форматом `'Mar 25'`. Разные форматы дат на разных страницах путают пользователя. Fix: экстрагировать `formatDateLong(iso): string` в `taskUtils.ts` (формат `month short + day + year`), импортировать в `TaskDetailSidebar`. `TaskDetailSidebar.tsx:28-33` | Siri | fixed |
| SIRI-UX-303 | minor | **`AgentPage.tsx` — две inline `new Date(x).toLocaleDateString()` без locale options**: в секции Memory (line 324) и History (line 388) даты форматируются через `toLocaleDateString()` без аргументов → выдаёт system locale format (M/D/YYYY в en-US, D.M.YYYY в ru). Несовместимо с остальным кодом который везде использует `{ month: 'short', day: 'numeric', year: 'numeric' }`. Fix: заменить оба места на `formatDateLong(x)` из taskUtils. `AgentPage.tsx:324,388` | Siri | fixed |
| SIRI-UX-304 | minor | **`SettingsPage.tsx` — `submitError` и `credentialsError` без `role="alert"`**: оба сообщения об ошибке появляются динамически (`<p data-testid="llm-credentials-error">` и `<div data-testid="credentials-fetch-error">`), но не имеют `role="alert"`. Screen reader не анонсирует их автоматически — пользователь не узнает о неудачной валидации ключа или ошибке загрузки credentials. Тот же паттерн что SIRI-UX-300, 301, 283. Fix: добавить `role="alert"` на оба элемента. `SettingsPage.tsx:359,380` | Siri | fixed |
| SIRI-UX-305 | minor | **`LibraryPortfolioPage.tsx:189` — `toLocaleDateString()` без locale options**: дата создания задачи в портфолио форматируется через `new Date(task.created_at).toLocaleDateString()` без аргументов → system-locale формат (M/D/YYYY en-US, D.M.YYYY ru). Несовместимо с остальным кодом который везде использует `formatDateLong()` из taskUtils. Fix: заменить на `formatDateLong(task.created_at)` + добавить импорт. `LibraryPortfolioPage.tsx:189` | Siri | fixed |
| SIRI-UX-306 | minor | **`AgentForm.tsx` — model validation error `<p>` без `role="alert"`**: при сабмите формы без выбранной модели появляется `<p style={{color:'#f87171'}}>Please select a model</p>` без `role="alert"`. Screen reader не анонсирует ошибку валидации. Тот же паттерн что SIRI-UX-300/301/304. Fix: добавить `role="alert"` и `data-testid="agent-form-model-error"` на `<p>`. `AgentForm.tsx:error paragraph` | Siri | fixed |
| SIRI-UX-307 | minor | **`KanbanBoard.tsx` — `TaskCard` подписывается на весь `tasks` array только для ре-рендера**: строка `useAgentStore((s) => s.tasks)` в `TaskCard` подписывает каждую карточку на любое изменение глобального списка задач. N карточек → N подписок → каждое изменение любой задачи (drag, run, edit) тригерит ре-рендер всех N карточек. Fix: убрать `useAgentStore((s) => s.tasks)` из `TaskCard` — мутации уже используют `useAgentStore.getState().tasks` (getState не подписывается), карточка перерендерится через родительский KanbanBoard при изменении filteredTasks. `KanbanBoard.tsx:~79` | Siri | fixed |
...
| BUG-075 | critical | **E2E prod флоу (conditional)**: шаги 1–6 (register → login → company → agent → task → run) работают ✅. **БЕЗ real LLM**: ран завершается с mock ответом `"Completed task at depth 1: {task_id}"`, `total_tokens=0`, War Room Activity Feed пуст — технически не падает, но ценности нет. **С real LLM**: заблокировано BUG-076 (ручная установка `AGENTCO_USE_REAL_LLM=true` + LLM API key в Railway — **требует action @timofeytst**). BUG-077 (CI) закрыт: tsc чистый, все 892 frontend + backend тесты зелёные. Единственный оставшийся блокер — BUG-076. | Alisa | open |
...
| ALEX-TD-194 | minor | **`services/agent.py:AgentService.delete` — N+1 UPDATE запросы при нуллификации `task.agent_id`**: Python loop `for task_orm in list(agent_orm.tasks): task_orm.agent_id = None` issuesN отдельных UPDATE запросов (один на каждую задачу агента). При агентах с сотнями задач — сотни round-trips до БД. Fix: заменить на bulk SQL UPDATE: `session.execute(update(TaskORM).where(TaskORM.agent_id == agent_id).values(agent_id=None))`. `services/agent.py:delete` | Alex | fixed |
| ALEX-TD-195 | minor | **`services/run.py:533` — `execute_run` outer except block missing `exc_info=True`**: `logger.error("execute_run failed for %s: %s", run_id, exc)` теряет traceback при неожиданных ошибках рана. В production-логах только тип исключения без стека — диагностика невозможна. Fix: добавить `exc_info=True`. `services/run.py:533` | Alex | fixed |
| ALEX-TD-196 | minor | **`services/run.py:261` — jitter реализован как +10%, а не "full jitter"**: комментарий «ALEX-TD-061: full jitter» вводит в заблуждение. `delay = delay + random.uniform(0, 0.1) * delay` даёт 100-110% от exponential backoff (±0-10%). Настоящий full jitter = `random.uniform(0, delay)` — распределяет 0..100%, значительно снижает thundering herd при 3+ ретраях. При delay=4s текущий spread: 4.0-4.4s; настоящий: 0-4.0s. Fix: `delay = random.uniform(0, delay)`. `services/run.py:261` | Alex | fixed |
| ALEX-TD-197 | minor | **`orchestration/agent_node.py:404` — tool handler exception missing `exc_info=True`**: `logger.error("Tool handler '%s' failed: %s", tool_name, e)` теряет traceback. Ошибки внутри tool handlers (database errors, network failures) логируются только типом исключения — traceback недоступен, сложно воспроизвести в production. Fix: добавить `exc_info=True`. `orchestration/agent_node.py:404` | Alex | fixed |
| ALEX-TD-198 | minor | **`main.py` — `/health` endpoint не rate-limited**: `GET /health` выполняет `SELECT 1` в БД на каждый запрос без ограничений. Load balancer + monitoring систем могут спамить по 100 req/s → 100 DB-запросов/сек без пользы. Для сравнения: `/api/health` аналогичного назначения не имеет rate limit тоже. Fix: добавить `@limiter.limit("120/minute")` к `health_check` и `health`. `main.py:health_check` | Alex | fixed |
| ALEX-TD-200 | minor | **`services/run.py` — `logger.error` в dead-letter пути без `exc_info=True`**: traceback теряется, post-mortem debugging невозможен. Fix: добавить `exc_info=True` в вызов `logger.error`. `services/run.py:271` | Alex | fixed |
| ALEX-TD-201 | minor | **`services/task.py:list_by_agent` вызывает `repo.list()` без `order_by`**: нет детерминированного порядка задач. Fix: делегировать в `repo.list_by_agent()` который применяет `ORDER BY created_at ASC`. `services/task.py:59` | Alex | fixed |
| ALEX-TD-202 | minor | **`services/run.py:406` — `initial_task_id` dead variable**: присваивается `run_orm.task_id` но нигде не используется — не попадает в `initial_state` и не логируется. Мёртвый код создаёт путаницу: кажется что task_id передаётся в граф, но это не так. Fix: удалить строку `initial_task_id = run_orm.task_id`. `services/run.py:406` | Alex | fixed |
| ALEX-TD-203 | minor | **`handlers/tasks.py:TaskOut` — не экспортирует `created_at` и `result`**: доменная модель `Task` содержит оба поля, но `TaskOut` их скрывает. Frontend не может отображать время создания задачи и её результат через REST API. Fix: добавить `created_at: datetime | None = None` и `result: str | None = None` в `TaskOut`. `handlers/tasks.py:68` | Alex | fixed |
| ALEX-TD-204 | major | **`services/run.py:405` — task-based run передаёт UUID task_id как goal в LangGraph**: когда ран создаётся через `POST /tasks/{task_id}/run`, `run_orm.goal` равен `None`. Строка `initial_goal = run_orm.goal or (run_orm.task_id or "")` подставляет UUID строку типа `"7f4a2b3c-..."` как `input` для LLM. CEO-агент получает UUID вместо описания задачи. Fix: загрузить `task.title + task.description` из БД и использовать как `initial_goal` когда `goal is None`. `services/run.py:405` | Alex | fixed |
| ALEX-TD-199 | minor | **`repositories/run.py:57` — ORDER BY `started_at.desc()` без `NULLS LAST`**: SQLite в DESC-сортировке помещает NULL-значения первыми. Сейчас `started_at` всегда устанавливается при создании, но без explicit `nulls_last()` поведение может сломаться при прямых DB-вставках или будущих изменениях модели. Fix: `.order_by(RunORM.started_at.desc().nullslast())` или добавить вторичную сортировку `RunORM.created_at.desc()`. `repositories/run.py:57,65` | Alex | fixed |
